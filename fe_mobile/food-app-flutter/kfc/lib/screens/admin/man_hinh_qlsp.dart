...
import 'package:kfc/services/product_service.dart';
import 'package:kfc/services/category_service.dart';
...
class _ManHinhQLSPState extends State<ManHinhQLSP> with SingleTickerProviderStateMixin {
  ...
  Widget _buildImage(String imagePath, {double? width, double? height, BoxFit? fit}) {
    ...
    // Chuẩn hóa URL để emulator Android truy cập được
    String resolvedPath = imagePath;
    if (!imagePath.startsWith('http') && imagePath.contains('/uploads/')) {
      resolvedPath = 'http://10.0.2.2:8080$imagePath';
    } else if (imagePath.startsWith('http')) {
      try {
        final uri = Uri.parse(imagePath);
        if (uri.host == 'localhost' || uri.host == '127.0.0.1') {
          resolvedPath = uri.replace(host: '10.0.2.2').toString();
        }
      } catch (_) {
        // giữ nguyên nếu parse thất bại
      }
    }
  ...
  // Upload ảnh lên backend (Multipart) -> trả về URL ảnh
  ...
  // Xóa ảnh trên backend (nếu có URL)
  Future<void> _deleteImage(String imageUrl) async {
    if (imageUrl.isEmpty || !imageUrl.contains('/uploads/')) return;
    try {
      final dio = DioClient.dio();
      await dio.delete('/files', queryParameters: {'url': imageUrl});
    } catch (e) {
      print('Lỗi khi xóa ảnh trên backend: $e');
    }
  }
        ...
        children: [
          ...
          Container(
            ...
            child: Material(
              ...
              child: InkWell(
                borderRadius: BorderRadius.circular(8),
                onTap: (_isLoadingProducts || _isLoadingCategories) ? null : () {
                  setState(() {});
                  _loadCategories();
                  _loadProducts();
                },
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      (_isLoadingProducts || _isLoadingCategories)
                          ? const SizedBox(
                              width: 16,
                              height: 16,
                              child: CircularProgressIndicator(
  ...
  Widget _buildProductList() {
    ...
    List<SanPham> products = List.from(_products);
                ...
                children: [
                  ...
                  if (selectedImage != null) ...[
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(8),
                        border: Border.all(color: MauSac.xanhLa, width: 2),
                      ),
                      child: ClipRRect(
                        borderRadius: BorderRadius.circular(8),
                        child: Image.file(
                            ...
                            return Container(
                              ...
                              child: const Center(
                                child: Column(
                                  mainAxisAlignment: MainAxisAlignment.center,
                                  children: [
                                    Icon(Icons.error_outline, color: MauSac.kfcRed, size: 48),
                                    SizedBox(height: 8),
                                    Text('Không thể hiển thị ảnh', style: TextStyle(color: MauSac.xam)),
                                  ],
                                ),
                              ),
                            );
  ...
  Future<void> _deleteProduct(SanPham product) async {
    try {
      if (product.hinhAnh.isNotEmpty && product.hinhAnh.contains('/uploads/')) {
