...
class _ManHinhXemDanhGiaState extends State<ManHinhXemDanhGia>
        ...
        children: [
          Row(
            children: [
              ...
              TextButton.icon(
                onPressed: () => _xoaDanhGiaCuaToi(),
                icon: const Icon(Icons.delete, size: 16, color: MauSac.cam),
                label: const Text(
                  'Xóa',
                  style: TextStyle(color: MauSac.cam),
                ),
              ),
            ],
          ),
  ...
  Future<void> _chinhSuaDanhGia() async {
    if (_danhGiaCuaToi == null) return;
    final result = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (context) => ManHinhVietDanhGia(
          sanPham: widget.sanPham,
          danhGiaCu: _danhGiaCuaToi,
        ),
      ),
    );
  ...
  Future<void> _xoaDanhGiaCuaToi() async {
    if (_danhGiaCuaToi == null) return;
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Xóa đánh giá'),
        content: const Text('Bạn có chắc muốn xóa đánh giá của mình?'),
        actions: [
          TextButton(
          ...
          TextButton(
            ...
            child: const Text('Xóa', style: TextStyle(color: MauSac.kfcRed)),
          ),
        ],
      ),
    );
    ...
    final ok = await DanhGiaService.xoaDanhGia(_danhGiaCuaToi!.id);
    if (!ok) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Row(
