...
class _ManHinhChiTietSanPhamState extends State<ManHinhChiTietSanPham> {
  ...
  Future<void> _taiDuLieuDanhGia() async {
    ...
    try {
      final thongKe = await DanhGiaService.layThongKeDanhGia(_sanPhamId);
      final danhGia = await DanhGiaService.layDanhGiaTheoSanPham(_sanPhamId);
      if (!mounted) return;
      setState(() {
        _thongKeDanhGia = thongKe;
        // lấy 3 đánh giá gần nhất
        _danhGiaGanNhat = danhGia.take(3).toList();
        _dangTaiDanhGia = false;
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _thongKeDanhGia = null;
        _danhGiaGanNhat = [];
        _dangTaiDanhGia = false;
      });
    }
  ...
  Future<void> _taiSanPhamLienQuan(SanPham sanPham) async {
    try {
      final danhSachLienQuan = await ProductService.laySanPhamLienQuan(sanPham);
      if (mounted) {
        setState(() {
...
// Hàm để xây dựng hình ảnh từ assets hoặc URL
  ...
  Widget _buildImageFromAssets(String imagePath, {double? height, BoxFit? fit}) {
    ...
    if (imagePath.startsWith('/')) {
      final fullUrl = 'http://10.0.2.2:8080$imagePath';
      return _buildImageFromNetwork(fullUrl, height: height, fit: fit);
    }
